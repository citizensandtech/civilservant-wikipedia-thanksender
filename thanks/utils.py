...
def _get_num_experiment_actions(db, experiment_id=None, action_subject_id=None):
    EAs_q = db.query(ExperimentAction)
    if experiment_id:
        EAs_q = EAs_q.filter(ExperimentAction.experiment_id == experiment_id)
    if action_subject_id:
        EAs_q = EAs_q.filter(ExperimentAction.action_subject_id == action_subject_id)
    q_count = EAs_q.count()
    return q_count
