...
class MaxSendExceededError(Exception):
    pass
...
class ThanksSender:
    ...
    def __init__(self, thank_batch_size=1, lang=None):
        ...
        self.max_send_errors = int(os.environ['CS_OAUTH_THANKS_MAX_SEND_ERRORS'])
    ...
    def find_thanks_needing_send(self):
        ...
        logging.info("staring query to find thanks needing")
    ...
    def actually_post_thanks(self, experiment_action):
        ...
        logging.info(f"Found the OAUTH user {oauth_user.username} who has autogenerated ID {oauth_user.id}.")
        access_token = oauth_user.metadata_json['access_token']
        access_token_key = access_token['key']
        access_token_secret = access_token['secret']
        logging.debug(f"ACCESS TOKENS key={access_token_key} secret={access_token_secret}")
        # This copied from:
    ...
    def run(self):
        ...
        for action in actions_needing:
            ...
            except MaxSendExceededError:
                logging.error(f"Found a MaxSendExceededError expAction id={action.id}")
                self.update_action_status(action)
            except Exception as e:
                logging.error(f"Outerloop is catching {e}")
